...
(function(factory) {
	if (typeof define === 'function' && define.amd) {
		define([ 'jquery', 'ztree', 'jqueryValidateAll', 'iconsetFontawesome',
				'iconPicker', 'elementsSpinner' ], factory);
	} else {
		factory(jQuery);
	}
}(function($) {
	...
	var operate = {
		checkForm : function() {
			 $("#menuInfoForm").validate(
				{
					...
					submitHandler : function(form) {
						$.ajax({
							...
							cache : false,
							error : function() {
								horizon.notice.error(horizon.lang['message']['operaterError']);
							},
            ...
            jQuery.validator.addMethod('menuName', function(value, element) {
                ...
                return true;
            }, horizon.lang['platform-menu']['validatorMenuName']);
		}
	};
	// 绑定各种事件
	var event = {
	...
	return horizon.manager['appmenu'] = {
		init : function() {
			menuTree.init();
			operate.checkForm();
			event.newMenu();
